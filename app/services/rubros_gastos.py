...
async def get_rubros(db: AsyncSession, solo_hojas: bool = False) -> list[RubroGasto]:
    stmt = select(RubroGasto)
    if solo_hojas:
...
async def get_rubro(db: AsyncSession, codigo: str) -> RubroGasto | None:
    result = await db.execute(select(RubroGasto).where(RubroGasto.codigo == codigo))
    return result.scalar_one_or_none()


async def buscar(db: AsyncSession, filtro: str) -> list[RubroGasto]:
...
async def saldo_disponible(db: AsyncSession, codigo_rubro: str) -> float:
    rubro = await get_rubro(db, codigo_rubro)
    if not rubro:
        return 0
    stmt = select(func.coalesce(func.sum(CDP.valor), 0)).where(
        and_(CDP.codigo_rubro == codigo_rubro, CDP.estado != "ANULADO")
    )
    result = await db.execute(stmt)
...
async def eliminar(db: AsyncSession, codigo: str):
    ...
    # Check for CDPs
    stmt = select(func.count()).select_from(CDP).where(CDP.codigo_rubro == codigo)
    result = await db.execute(stmt)
    if result.scalar() > 0:
