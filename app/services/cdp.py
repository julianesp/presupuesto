...
async def saldo_cdp(db: AsyncSession, numero_cdp: int) -> Decimal:
    cdp = await get_cdp(db, numero_cdp)
    if cdp is None:
        raise ValueError(f"CDP {numero_cdp} no encontrado")

    stmt = select(func.sum(RP.valor)).where(
        and_(RP.cdp_numero == numero_cdp, RP.estado != "ANULADO")
    )
...
async def registrar(
    ...
    saldo_rubro = await rubros_svc.saldo_disponible_rubro(db, codigo_rubro)
    if Decimal(str(valor)) > Decimal(str(saldo_rubro)):
        raise ValueError(
            f"El valor ({valor:,.2f}) supera el saldo disponible del rubro ({saldo_rubro:,.2f})"
        )

    numero = await config_svc.siguiente_consecutivo(db, "CDP")
    ...
    await guardar_concepto(db, codigo_rubro, objeto)
...
async def get_cdps(db: AsyncSession, estado: str | None = None) -> list[CDP]:
    stmt = select(CDP).order_by(CDP.numero.desc())
...
async def get_cdp(db: AsyncSession, numero: int) -> CDP | None:
    stmt = select(CDP).where(CDP.numero == numero)
    result = await db.execute(stmt)
    return result.scalar_one_or_none()


async def get_cdps_por_rubro(db: AsyncSession, codigo_rubro: str) -> list[CDP]:
    stmt = (
        select(CDP)
        .where(and_(CDP.codigo_rubro == codigo_rubro, CDP.estado != "ANULADO"))
...
async def actualizar_estado(db: AsyncSession, numero: int) -> CDP:
    cdp = await get_cdp(db, numero)
    if cdp is None:
        raise ValueError(f"CDP {numero} no encontrado")
    ...
    saldo = await saldo_cdp(db, numero)
...
async def editar(
    ...
    if nuevo_valor is not None:
        ...
        if diferencia > 0:
            saldo_rubro = await rubros_svc.saldo_disponible_rubro(db, cdp.codigo_rubro)
