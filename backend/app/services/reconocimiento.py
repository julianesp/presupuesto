...
async def get_reconocimientos(
    ...
    stmt = select(Reconocimiento).order_by(Reconocimiento.numero.desc())
...
async def get_reconocimiento(db: AsyncSession, numero: int) -> Reconocimiento | None:
    result = await db.execute(
        select(Reconocimiento).where(Reconocimiento.numero == numero)
    )
...
async def registrar(
    ...
    rubro = await rubros_svc.get_rubro(db, data.codigo_rubro)
    if rubro is None:
        raise ValueError(f"Rubro de ingreso {data.codigo_rubro} no encontrado")

    numero = await config_svc.get_consecutivo(db, "reconocimiento")
...
async def editar(db: AsyncSession, numero: int, data) -> Reconocimiento:
    rec = await get_reconocimiento(db, numero)
    if rec is None:
...
async def anular(db: AsyncSession, numero: int) -> Reconocimiento:
    rec = await get_reconocimiento(db, numero)
    if rec is None:
        raise ValueError(f"Reconocimiento {numero} no encontrado")
    if rec.estado == "ANULADO":
        raise ValueError("El reconocimiento ya está anulado")
